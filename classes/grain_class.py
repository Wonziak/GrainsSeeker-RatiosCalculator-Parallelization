    ...
    def __find_vector_perpendicular(self):
        ...
        for edgePoint1 in self.edge:
            ...
            for edgePoint2 in self.edge:
                if edgePoint1[0][0] == edgePoint2[0][0] and edgePoint1[0][1] == edgePoint2[0][1]:
                    continue
                vector = [edgePoint2[0][0] - edgePoint1[0][0], edgePoint2[0][1] - edgePoint1[0][1]]
                vector_length = math.sqrt(math.pow(vector[0], 2) + math.pow(vector[1], 2))

                numerator = ((vector[0] * self.maxDistanceVectorCoords[0]) + (
                        vector[1] * self.maxDistanceVectorCoords[1]))
                denominator = vector_length * self.maxDistancePoints
